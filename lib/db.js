    ...
    _createTable(tables, callback) {
        ...
        if (!Array.isArray(tables)) {
            ...
            for (let i = 0; i < names.length; ++i) {
                const name = names[i];
                options[name] = (!tables[name] ? false : (tables[name] === true ? {} : tables[name]));
            }
        }
        ...
        RethinkDB.db(this._name).tableList().run(this._connection, (err, existing) => {
            ...
            const each = (name, next) => {
                ...
                if (existing.indexOf(name) === -1) {
                    ...
                    RethinkDB.db(this._name).tableCreate(name).run(this._connection, (err, result) => {

                        this.table(name, options[name]);
                        return next(err);
                    });
                }
                else {
                    ...
                    this.table(name, options[name]);
                    return this.tables[name].empty(next);
                }
            };
