...
exports = module.exports = class {
    ...
    _normalizeTables(tables, options) {
        ...
        if (typeof tables === 'string' ||
            ...
            options = Joi.attempt(options || {}, internals.schema.table, 'Invalid table options');
        ...
        names.forEach((name) => {
            ...
            tables[name] = Joi.attempt(tables[name], internals.schema.table, `Invalid table options: ${name}`);
        });
    ...
    verify(callback) {
        ...
        const each = (table, next) => Id.verify(table, next);
        Items.serial(verify, each, callback);
    }
