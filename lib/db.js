    ...
    _createTable(tables, callback) {
        ...
        RethinkDB.db(this.name).tableList().run(this._connection, (err, existing) => {
            ...
            const each = (name, next) => {
                ...
                const createOptions = Hoek.applyToDefaults({}, {
                    primaryKey: tableOptions.primaryKey
                });

                if (existing.indexOf(name) === -1) {
                    return RethinkDB.db(this.name).tableCreate(name, createOptions).run(this._connection, (err) => finalize(err, tableOptions.secondary));
                }
